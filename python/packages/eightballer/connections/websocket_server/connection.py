...
from packages.eightballer.protocols.websockets.dialogues import WebsocketsDialogue
...
class WebSocketChannel(HTTPChannel):
    ...
    async def connect(self, loop: AbstractEventLoop) -> None:
        ...
        if self.is_stopped:
            ...
            try:
                ...
                self.logger.info(
                    "WebSocket Server has connected to port: {}.".format(self.port)
                )
    ...
    async def _http_handler(self, http_request: BaseRequest):
        ...
        """
        self.logger.info(
            f"Handling initial http request for websocket connection from {http_request.remote}"
        )
        ...
        try:
            ...
            http_envelope = request.to_envelope_and_set_id(
    ...
    async def _inbound_wss_handler(
        self, http_request: BaseRequest, websocket_message: WSMessage
    ):
        """
        Verify the request then send the request to Agent as an envelope.
        ...
        self.logger.debug(
            f"Received inbound message from websocket client: {websocket_message}"
        )
    ...
    async def _handle_new_client(
        self, http_request: BaseRequest = None, url=None, sid=None, sio=None, ws=None
    ) -> None:
        """
    ...
    async def _start_ws_server(self) -> None:
        ...
        async def connect(sid, environ):
            ...
            msg, _ = await self._handle_new_client(
                sid=sid, url=environ["PATH_INFO"], sio=sio
            )
            await self.send_msg_to_agent(msg)
    ...
    async def send(self, envelope: Envelope) -> None:  # pylint: disable=W0236
        ...
        if envelope.protocol_specification_id == HttpMessage.protocol_id:
            ...
            raise ValueError(
                f"Unsupported protocol specification id: {envelope.protocol_specification_id}"
            )
    ...
    async def send_message_to_client(self, message, dialogue) -> Optional[Envelope]:
        ...
        ws_dialogues_to_connections = {
            v.incomplete_dialogue_label: k for k, v in self.pending_requests.items()
        }
        if message.performative == WebsocketsMessage.Performative.SEND:
            http_dialogue_ref = ws_dialogues_to_connections.get(
                dialogue.incomplete_dialogue_label, None
            )
            if http_dialogue_ref is None:
                self.logger.warning(
                    "Could not locate http dialogue for message={}".format(message)
                )
                msg = dialogue.reply(
                    performative=WebsocketsMessage.Performative.DISCONNECT,
                    reason="Could not locate http dialogue for message={}".format(
                        message
                    ),
                )
                await self.send_msg_to_agent(msg)
